...
function createSidebarData(sourceDir, collapsable) {
    ...
    tocs.forEach(toc => { // toc是每个目录的绝对路径
        ...
        if (toc.substr(-6) === '_posts') { // 碎片化文章
            ...
            const sidebarObj = mapTocToSidebar(toc, collapsable,0);
...
function mapTocToSidebar(root, collapsable, prefix = '',order) {
    ...
    files.forEach(filename => {
        ...
        if (isDir) { // 是文件夹目录
            ...
            //截取文件夹的名称，去掉序号
            title =fileNameArr[length-1];


            sidebar[order] = {
                title,
                collapsable, // 是否可折叠，默认true
                children: mapTocToSidebar(file, collapsable, prefix + filename + '/',order).sidebar // 子栏路径添加前缀
            }
