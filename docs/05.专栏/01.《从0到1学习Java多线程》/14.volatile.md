...
如果 ThreadA、ThreadB 同时往主内存读值，拿到count是1，ThreadA修改count的值为2，然后根据volatile，立马往主内存回写（保证了可见性），但是ThreadB 又自增，但是ThreadB 一开始读到的count也是1，回写2，这样子就会导致两个线程都执行了，但是count的值是2，而不是3。（不能保证原子性）
...
- 线程 A 执行 1、3：`instance` 不为 null，但对象**还没初始化**
- 线程 B 外层判断 `instance != null`，直接返回**半初始化对象** → 报错。



`volatile` 禁止 **1→3 重排**，保证初始化完成后，引用才赋值，DCL 线程安全。
...
```java
// 不加 volatile，主线程修改 flag，子线程可能永远感知不到
...
public static void main(String[] args) throws InterruptedException {
    ...
    Thread.sleep(1000);
    flag = true; // 写 volatile
}
```

加 `volatile`：子线程能及时看到 `flag=true`，循环退出。

不加：JIT 优化后子线程会一直读取本地缓存，**死循环**。



## volatile的内存语义

### 语义规则（JMM 对 volatile 变量的读写约定）

1. **写 volatile 变量**

   线程修改 `volatile` 变量后，**强制立刻把工作内存的新值刷新到主内存**。

2. **读 volatile 变量**

   线程读取 `volatile` 变量前，**强制清空当前工作内存的缓存，重新从主内存加载最新值**。

### volatile 禁止重排的核心：**内存屏障（Memory Barrier）**

JMM 靠**插入内存屏障**禁止指令跨越 `volatile` 读写位置重排，一共四类屏障：

- `LoadLoad `   读 - 读屏障，禁止「前面的读操作」和「后面的读操作」互相重排。
  ...
  加了 LoadLoad 后：读A 一定先执行，再执行 读B
  绝对不允许变成：读B → 读A
  ```

- `StoreStore ` 写 - 写屏障，禁止「前面的写操作」和「后面的写操作」互相重排
  ...
  加了 StoreStore 后：写A 一定先执行完，再执行 写B
  绝对不允许变成：写B → 写A
  ```

- `LoadStore`  读 - 写屏障，禁止「前面的读操作」和「后面的写操作」互相重排。

- `StoreLoad `  写 - 读屏障，禁止「前面的写操作」和「后面的读操作」互相重排。（开销最大）
...
它会强制**清空 CPU 写缓冲、刷新缓存、等待数据落主存**，做到全局数据可见，同步成本最高。
