...
```mermaid
flowchart TD
    A[提交新任务] --> B{当前线程数<br> < corePoolSize?}
    B -- 是 --> C[创建新核心线程<br>执行任务]
    B -- 否 --> D{任务队列<br>是否已满?}
    D -- 否 --> E[任务入队等待]
    D -- 是 --> F{当前线程数<br> < maxPoolSize?}
    F -- 是 --> G[创建新临时线程<br>执行任务]
    F -- 否 --> H[触发拒绝策略]
```
...
总结：**核心线程跑满了 → 任务进队列 → 队列也满了 → 还没达到最大线程数，就创建临时线程执行 → 连临时线程也达到上限了，再来新任务就触发拒绝策略。**

> ❗❌  这里说的只是瞬时状态转换逻辑，拒绝策略的触发，是在**任务提交的那一瞬间**，一个任务在被提交时，只要**当前瞬间**有**空闲的线程**，或者**队列有空间**，这个任务就会被接受并处理，不会触发拒绝。
>
> 只要任务执行完成够快，任务提交够慢，完全也可能不会阻塞，不触发拒绝策略。
...
| 策略类                | 名称                     | 执行逻辑                                                     | 适用场景                                   |
| --------------------- | ------------------------ | ------------------------------------------------------------ | ------------------------------------------ |
| `AbortPolicy`         | 中止策略（**默认策略**） | 直接抛出 `RejectedExecutionException` 运行时异常，丢弃任务   | 任务不能丢失，需要捕获异常做降级 / 重试    |
| `CallerRunsPolicy`    | 调用者运行策略           | 不新开工作线程，由**提交任务的主线程**执行该任务；线程池已关闭则丢弃 | 不允许丢任务，可接受提交线程阻塞、削峰限流 |
| `DiscardPolicy`       | 丢弃策略                 | 静默丢弃当前新来任务，无异常、无日志                         | 任务不重要，允许丢失，对数据无强依赖       |
| `DiscardOldestPolicy` | 丢弃最旧任务策略         | 弹出阻塞队列队头存放最久的任务，再尝试提交当前新任务         | 新任务优先级更高，需要保证最新任务执行     |

 具体触发场景

| 场景                                         | 触发条件                                         | 示例                                                         |
| :------------------------------------------- | :----------------------------------------------- | :----------------------------------------------------------- |
| **核心线程满载 + 队列满 + 临时线程已达上限** | 达到 maxPoolSize，队列也满了                     | core=2，queue=5，max=3，第9个任务触发                        |
| **线程池已关闭**                             | 调用 `shutdown()` 或 `shutdownNow()` 后提交任务  | 即使线程池中有空闲线程，也会触发拒绝策略                     |
| **队列有界且容量小**                         | 队列是 `ArrayBlockingQueue` 等有界队列，且填满了 | 如果队列是无界的（如 `LinkedBlockingQueue` 无参构造），不会触发，因为永远不会“满” |
...
> main 线程发现核心线程池+队列已经满了，就立马抛出错误，然后子线程才开始执行（也未必,可能子线程执行更快）
