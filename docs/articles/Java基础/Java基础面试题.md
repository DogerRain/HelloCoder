...
| 普通字段声明 | 抽象类的字段声明可以是任意的，但是不能是static，因为抽象类是要被实现的，静态类是不能被修改的；可以使用final修饰 | 默认是public static final（只能是这三个关键字，或其中两个/一个或都省略） |
| 普通方法声明 | 同上                                                         | 1.8中及以后，也可以普通方法了（表示final、static都可以修饰了，和普通类无异） |
| 实例化       | 不是具体类，不能实例化                                       | 不是类，不能实例化                                           |
| 方法体       | 抽象方法访问类型不能是private                                | 接口中方法默认是public abstract（只能是这两个关键字，或其中一个或都省略）。因为要被继承，所以是public的。 |
