...
| 情况               | 原因                                  | 解决                       |
| :----------------- | :------------------------------------ | :------------------------- |
| 关联字段类型不一致 | `int` vs `varchar`                    | 统一数据类型               |
| 关联字段字符集不同 | `utf8` vs `utf8mb4`                   | 统一字符集                 |
| 使用函数或表达式   | `DATE(create_time) = '2026-01-01'`    | 改为 `create_time BETWEEN` |
| 隐式类型转换       | `user_id = '123'`（`user_id` 是 int） | 传正确的类型               |