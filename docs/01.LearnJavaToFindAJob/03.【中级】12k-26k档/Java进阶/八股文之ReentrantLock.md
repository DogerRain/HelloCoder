...
**核心原则：** 插队机制，允许新来的线程与队列头部线程一起竞争锁。

工作流程：

1. 新线程上来直接 CAS 插队抢锁，不检查等待队列（非公平插队）, 
2. **CAS 插队成功**：直接持有锁执行代码，state 由 0 变 1 
3. CAS 插队失败：再次尝试及进入 AQS 核心流程，线程会进入 AQS 的 `acquire(1)`
4. AQS 失败，将线程包装成Node加入等待队列，进入自旋，不断尝试获取锁或挂起等待
5. 在队列中自旋竞争，长期抢不到就挂起；被唤醒后继续竞争，直到获取锁。
...
**核心原则：** 先到先得，严格按照请求锁的顺序来分配锁。

**工作流程：**

1. 线程调用 `lock()` 时，然后执行tryAcquire(1)，首先检查 **同步队列（CLH 队列）** 中是否有其他线程在等待
2. **如果有等待线程**：当前线程直接进入队列尾部排队，不参与竞争
3. **如果没有等待线程**：尝试 CAS 获取锁
4. 锁释放时，总是唤醒队列头部的下一个线程
