...
JMM（Java 内存模型）本质就是一套**跨硬件、跨操作系统的统一内存访问规范**，用来屏蔽 CPU 架构、缓存协议、指令集带来的内存行为差异。
...
- **主内存**：java虚拟机规定所有的变量都必须在主内存（RAM）中产生，为了方便理解，可以认为是**堆区**。可以与前面说的物理机的主内存（RAM）相比，只不过物理机的主内存（RAM）是整个机器的内存，而虚拟机的主内存是虚拟机内存中的一部分。
- **工作内存**：java虚拟机中每个线程都有自己的工作内存（JMM抽象出来的），该内存是线程私有的为了方便理解，可以认为是**虚拟机栈**。可以与前面说的高速缓存相比。线程的工作内存保存了线程需要的变量在主内存中的副本。虚拟机规定，线程对主内存变量的修改必须在线程的工作内存中进行，不能直接读写主内存中的变量。不同的线程之间也不能相互访问对方的工作内存。如果线程之间需要传递变量的值，必须通过主内存来作为中介进行传递。
...
##  5、总结
 ...
 Java 是较早定义内存模型（JMM）的编程语言，设计初衷是**屏蔽不同 CPU、操作系统的底层内存差异，简化多线程开发、保障代码跨平台可移植**。

1. CPU 硬件层面依靠 **MESI 等缓存一致性协议**，解决多核 CPU 缓存数据不一致问题；但受写缓冲、无效队列影响，MESI 只能保障数据最终一致性，无法解决指令重排带来的多线程可见性乱象。
2. 编译器、CPU 为优化执行效率会进行**指令重排序**：单线程环境下重排不会改变代码执行结果（串行语义守恒），但多线程场景下，指令乱序会破坏数据可见性与执行有序性，引发并发异常。
3. **JMM（Java 内存模型）** 是 Java 规范定义的一套并发准则：抽象出「工作内存 - 主内存」访问模型，约束源码编译、指令重排、内存读写行为，向上给开发者统一的并发编程规范，向下屏蔽软硬件差异，实现一次编写随处运行。
4. JSR-133 规范引入 **happens-before（先行发生）** 规则，作为判定多线程之间数据是否可见、指令能否重排的理论依据。

一句话总览：

硬件靠 MESI 保证缓存一致，CPU / 编译器存在指令重排隐患，JMM 通过统一规范 + happens-before 规则屏蔽底层差异、约束重排与可见性，实现 Java 多线程跨平台。
