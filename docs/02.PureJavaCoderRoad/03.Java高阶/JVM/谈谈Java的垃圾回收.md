...
| **算法名称**                 | **核心思想**                                                 | **优缺点**                                              | **主要应用场景**              |
| ---------------------------- | ------------------------------------------------------------ | ------------------------------------------------------- | ----------------------------- |
| **标记-清除 (Mark-Sweep)**   | 先标记所有可回收对象，然后统一清除。                         | 速度快，但会产生大量**内存碎片**。                      | 老年代（Old Gen）的早期版本。 |
| **复制 (Copying)**           | 将内存分为大小相等的两块，每次只使用一块。存活对象复制到另一块。 | **无碎片**，但内存利用率只有 50%。                      | **新生代 (Young Gen)**。      |
| **标记-整理 (Mark-Compact)** | 标记存活对象后，将所有存活对象移动到一端，然后清理边界外的内存。 | **无碎片**，但移动对象需要暂停用户线程（STW），开销大。 | 老年代。                      |
